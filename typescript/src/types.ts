...
/**
 * Single transcoded video rung. Generated by the storage CDN when
 * `transcodeVideo: true` is passed to `media.upload`. Reachable via
...
/**
 * Async background-processing tracker. Populated only on uploads
 * that opted into the multi-quality video ladder. Polling pattern:
 *
 *   while ((m = await sentroy.media.get(bucket, id)).processing &&
...
export interface Media {
  ...
  /** Populated for video uploads with `transcodeVideo: true`.
   *  Variant rungs stream in over time as the background pipeline
   *  finishes each ladder step. */
  videoMeta?: MediaVideoMeta
  /** Async background processing tracker. Present on any upload
   *  that triggered async work (currently only video transcoding).
   *  Treat anything other than `"completed"` / `undefined` as
   *  "still in flight". */
...
export interface UploadMediaParams {
  ...
  tags?: string[]
  /**
   * Light single-pass H.264 re-encode of the source video at the
   * source resolution. Trims a typical phone/screen-recording
   * upload by 30–60% with no visible quality loss. Roughly doubles
   * the upload-handler latency since the request waits for the
   * encode before returning. Default `false`. Ignored on non-video
   * uploads.
   */
  compressVideo?: boolean
  /**
   * Multi-quality variant ladder (144p / 480p / 720p / 1080p).
   * Implies `compressVideo`. Variants are generated **asynchronously**
   * after the upload response: the API returns immediately with
   * `media.processing.status === "queued"`, then streams ladder
   * rungs into `media.videoMeta.variants` one by one. Poll the
   * media list (or fetch by id) to watch `variantsCompleted` climb
   * — once `processing.status === "completed"` the ladder is fully
   * available via `/f/<id>/<height>` (e.g. `/f/abc/720`). Default
   * `false`. Ignored on non-video uploads.
   */
